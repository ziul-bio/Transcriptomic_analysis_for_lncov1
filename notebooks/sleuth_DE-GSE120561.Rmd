...
base_dir <- "../results/GSE120561/kallisto/quant"
...
control vs treated, control will be used as first level). 
...
write_xlsx(sleuth_results_Q, path='../results/GSE120561/sleuth/results_sleuth_W_x_Q_Ducan_et.al.xlsx')
```
...
#ggsave(file="results/volcano1_queen.jpeg", height=5, width=9, dpi=300)
```
...
volcano + geom_label_repel(data= legen, aes(label= legen$target_id), size=4, fontface = "italic",
  ...
  labs(title= "Volcano plot of Diferencial Expressed Transcipts",
       ...
       color="Differentially Expressed") +
  theme_bw() + coord_cartesian(clip = "off")
#ggsave(file="results/volcano2_queenless.jpeg", height=5, width=9, dpi=300)
```
...
plot_grid(plot1,
          ...
          labels = c("A", "B"),
          ncol = 1, nrow = 2)
ggsave(file="../results/GSE120561/figures/boxplot_lncov1_Ef1a.jpeg", height=8, width=9, dpi=600)
```
